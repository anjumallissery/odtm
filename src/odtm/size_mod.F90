...
module size_mod
  ...
  integer, parameter :: nLayer=06, km=nLayer, lm=13 !added by anju 08Aug2020
  ...
  !anju 02Aug2020       
  real, pointer, dimension(:,:,:,:) :: nit_read => null(), sil_read => null()
  !*************************
  ...
  real, pointer, dimension(:) :: phi => null(), theta => null(), tan_theta => null()
    ...
    subroutine init_size()
        ...
        !added by anju 30June 2020**********
        allocate ( nit_read(isc:iec,jsc:jec,kmaxMYM,lm) )
        allocate ( sil_read(isc:iec,jsc:jec,kmaxMYM,lm) ) !doubt = kmax or kmaxMYM
        !***********************************
