...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
    ...
    use size_mod, only : isd, ied, jsd, jed, dxv
    use param_mod, only : day2sec, diffuse_h, diffuse_th, dphi, dt, dthe, deg2rad
    ...
    logical :: NaN_check
    ! For global sums
    real :: dudx_g, dvdy_g, wd_g, we_g, diffuse_g, rmass_g, rmass_b, rgrid_g
    character(len=1024) :: msg
    logical :: debug=.false.
    real rUp, rDw, h_mix

    dudx_g = 0; dvdy_g=0; wd_g=0; we_g=0; diffuse_g=0; rmass_g = 0; rmass_b = 0; rgrid_g = 0
...
!                dthe = (rdy(j) + rdy(j+1))/2.0
                ...
                rtemp3 = v(i,jp,k,taun) * sum_2pt (3,i,j,i,jp) *rUp / sum_2pt (31,i,j,i,jp)
                rtemp4 = v(i,j,k,taun) * sum_2pt (3,i,j,i,jm)  *rDw/ sum_2pt (31,i,j,i,jm)
                !tropdvbdy = ( rtemp3 - rtemp4 ) * rdyh(i,j) / cos(theta(j))
                !tropdvbdy = ( rtemp3 - rtemp4 ) * rdyh(i,j) 
                tropdvbdy = ( rtemp3 - rtemp4 ) * rdyh(i,j) * rdxh(i,j)
                ...
                dudx_g = dudx_g + rkmh(i,j) * tropdubdx * cos(theta(j))
                dvdy_g = dvdy_g + rkmh(i,j) * tropdvbdy * cos(theta(j))

                we_g   = we_g + rkmh(i,j) * we(i,j,k)
                wd_g   = wd_g + rkmh(i,j) * wd(i,j,k)
                diffuse_g = diffuse_g + rkmh(i,j) * diffuse_h * diffuse_th
    ...
    if (debug) then
        ...
        call mpp_sum(we_g)
        call mpp_sum(wd_g)
        call mpp_sum(diffuse_g)

        write(msg,*) "Global Sum of dudx, dvdy, we, wd, diffuse, total: ", &
                     dudx_g, dvdy_g, we_g, wd_g, diffuse_g, dudx_g+dvdy_g
